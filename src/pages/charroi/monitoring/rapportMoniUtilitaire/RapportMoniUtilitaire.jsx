...
const RapportMoniUtilitaire = () => {
  ...
  const [idDevice, setIdDevice] = useState(null); // aucune sélection par défaut
  ...
  const fetchData = async (from, to) => {
    if (!idDevice) return; // rien à faire si aucun véhicule sélectionné
    try {
      setLoading(true);
                ...
                >
                  ...
                  >
                    ...
                    <Space direction="vertical" size={2}>
                      ...
                      <Text>
                        <ClockCircleOutlined /> <strong>Durée moteur :</strong> {formatSecondsToTime(event.engine_work)}
                      </Text>
                      {event.items?.[0] && (
                        <Text>
